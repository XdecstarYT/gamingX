/* ============================================================================
   DOMINION — UI layer: map renderer + dashboard panels.
   Pure rendering + DOM wiring. Reads/mutates `state` via DomSim's action
   functions; never touches simulation internals directly.
   ========================================================================== */
window.DomUI = (() => {
'use strict';
const D = window.DomData;
const S = window.DomSim;
const W = window.DomWorld;
const L = window.DomLegislature;
...
let attackFraction = 0.5;      // share of the garrison committed to an attack
...
function handleMapClick(e) {
  ...
  if (armedBuild) {
    ...
    if (currentTab === 'territory') renderDash();
  }
}
...
function toast(text, kind) {
  ...
  el.className = 'dm-toast' + (kind ? ' ' + kind : '');
  el.textContent = text;
...
function playToastSound(text, kind) {
  const A = window.DomAudio; if (!A) return;
  ...
  if (kind === 'battle') {
    if (/captured territory from|eliminated from the map/.test(text)) A.fanfare();
    else if (/repelled|seized territory from you/.test(text)) A.negative();
    else A.warn();
    return;
  }
  A.click();
...
function setTab(tab) {
  currentTab = tab;
  document.querySelectorAll('.dm-tab').forEach(t => t.classList.toggle('on', t.dataset.tab === tab));
  renderDash();
}

function sparkline(values, color) {
  const c = document.createElement('canvas');
  c.className = 'dm-sparkline'; c.width = 300; c.height = 88;
  const cx = c.getContext('2d');
...
function renderOverview(body) {
  const s = state.stats;
  const g1 = document.createElement('div'); g1.className = 'dm-group';
  g1.innerHTML = `<b class="hd">NATION SNAPSHOT</b>
    <div class="dm-stat-grid">
      <div class="dm-stat-card"><div class="v">${fmt(s.population)}</div><div class="l">POPULATION</div></div>
      <div class="dm-stat-card"><div class="v">${Math.round(s.happiness)}</div><div class="l">HAPPINESS</div></div>
      <div class="dm-stat-card"><div class="v">${Math.round(s.corruption)}</div><div class="l">CORRUPTION</div></div>
      <div class="dm-stat-card"><div class="v">${s.inflation.toFixed(1)}%</div><div class="l">INFLATION</div></div>
      <div class="dm-stat-card"><div class="v">${s.unemployed}</div><div class="l">UNEMPLOYED</div></div>
      <div class="dm-stat-card"><div class="v">${state.tech.unlocked.length}</div><div class="l">TECHS</div></div>
    </div>`;
  body.appendChild(g1);

  const g2 = document.createElement('div'); g2.className = 'dm-group';
  g2.innerHTML = `<b class="hd">GDP OVER TIME</b>`;
  g2.appendChild(sparkline(s.gdpHistory, '#22d3ee'));
  body.appendChild(g2);

  const g3 = document.createElement('div'); g3.className = 'dm-group';
  g3.innerHTML = `<b class="hd">APPROVAL OVER TIME</b>`;
  g3.appendChild(sparkline(s.approvalHistory, '#a855f7'));
  body.appendChild(g3);

  const g4 = document.createElement('div'); g4.className = 'dm-group';
  g4.innerHTML = `<b class="hd">RECENT NEWS</b><div class="dm-feed">` +
    state.notifications.slice(-8).reverse().map(n => `<div class="dm-feed-item">${esc(n.text)}</div>`).join('') +
    `</div>`;
  body.appendChild(g4);
}
...
function renderBuild(body) {
  const isGov = state.legislature.isPlayerGovernment;
  if (!isGov) {
    const note = document.createElement('div'); note.className = 'dm-locked-note';
  ...
  for (const [cat, label] of cats) {
    ...
    const catEl = document.createElement('div'); catEl.className = 'dm-build-cat'; catEl.textContent = label;
    body.appendChild(catEl);
    for (const def of items) {
      const locked = def.needsTech && !S.techUnlocked(state, def.needsTech);
      const el = document.createElement('button');
      el.className = 'dm-build-item' + (locked || !isGov ? ' disabled' : '');
...
function renderEconomy(body) {
  ...
  for (const [k, label] of [['income', 'Income Tax'], ['corporate', 'Corporate Tax'], ['sales', 'Sales Tax']]) {
    const row = document.createElement('div'); row.className = 'dm-slider-row';
  ...
  g3.innerHTML = `<b class="hd">BUDGET (per week)</b>
    <div class="dm-budget-row"><span>Tax revenue</span><span>+${fmtMoney(s.lastTaxRevenue || 0)}</span></div>
    <div class="dm-budget-row"><span>Upkeep &amp; interest</span><span>−${fmtMoney(s.lastUpkeep || 0)}</span></div>
    <div class="dm-budget-row total"><span>Net</span><span>${(s.netIncome || 0) >= 0 ? '+' : ''}${fmtMoney(s.netIncome || 0)}</span></div>`;
  body.appendChild(g3);
...
function renderPolitics(body) {
  ...
  g1.innerHTML = `<b class="hd">GOVERNMENT</b>
    <div class="dm-stat-grid">
      <div class="dm-stat-card"><div class="v">${gov.name}</div><div class="l">TYPE</div></div>
      <div class="dm-stat-card"><div class="v">${Math.round(s.approval)}%</div><div class="l">APPROVAL</div></div>
    </div>
  ...
  const bar = document.createElement('div'); bar.className = 'dm-party-bar';
  const legend = document.createElement('div'); legend.className = 'dm-party-legend';
  for (const p of state.parties) {
    const def = D.PARTIES.find(x => x.id === p.id);
    const seg = document.createElement('div'); seg.className = 'dm-party-seg'; seg.style.width = p.support + '%'; seg.style.background = def.color;
    bar.appendChild(seg);
  ...
  g3.innerHTML = `<b class="hd">ACTIVE LAWS (${activeLaws.length})</b>` +
    (activeLaws.length
      ? activeLaws.map(l => `<div class="dm-law-item"><span class="info"><span class="nm">${esc(l.name)}</span><span class="ds">${esc(l.desc)}</span></span></div>`).join('')
...
function renderTech(body) {
  for (const cat of D.TECH_CATEGORIES) {
    const wrap = document.createElement('div'); wrap.className = 'dm-tech-cat';
    ...
    for (const t of D.TECHS.filter(x => x.cat === cat)) {
      ...
      card.className = 'dm-tech-card ' + (unlocked ? 'unlocked' : researching ? 'researching' : available ? '' : 'locked');
  ...
  const rp = document.createElement('div'); rp.className = 'dm-group';
  rp.innerHTML = `<b class="hd">RESEARCH POINTS</b><div class="dm-stat-grid"><div class="dm-stat-card"><div class="v">${fmt(state.stats.researchPoints)}</div><div class="l">STORED</div></div><div class="dm-stat-card"><div class="v">+${state.stats.researchPerTick}</div><div class="l">PER WEEK</div></div></div>`;
...
function renderDiplomacy(body) {
  ...
  for (const n of state.nations) {
    const card = document.createElement('div'); card.className = 'dm-nation-card';
    card.innerHTML = `
    ...
    if (n.eliminated) {
      const note = document.createElement('p'); note.style.cssText = 'color:var(--muted);font-size:.74rem;text-align:center;padding:6px';
      note.textContent = 'This nation has been eliminated.';
      card.appendChild(note);
      body.appendChild(card);
      continue;
    }
    const actions = document.createElement('div'); actions.className = 'dm-nation-actions';
    const acts = n.atWar
      ? [['peace', 'Sue for Peace', false]]
      : [
    ...
    if (n.atWar) {
      const warNote = document.createElement('p'); warNote.style.cssText = 'color:#f87171;font-size:.7rem;font-weight:800;margin-bottom:6px';
      warNote.textContent = '⚔️ AT WAR — attack their territory from the Territory tab.';
      card.appendChild(warNote);
    }
...
function renderParliament(body) {
  ...
  const banner = document.createElement('div'); banner.className = 'dm-gov-banner' + (isGov ? '' : ' opposition');
  ...
  const chart = document.createElement('div'); chart.className = 'dm-seat-chart';
  const legend = document.createElement('div'); legend.className = 'dm-party-legend';
  for (const p of D.PARTIES) {
    const count = seatCounts[p.id] || 0;
    if (!count) continue;
    const seg = document.createElement('div'); seg.className = 'dm-seat-seg';
  ...
  for (const pf of D.PORTFOLIOS) {
    ...
    const card = document.createElement('div'); card.className = 'dm-cabinet-card';
  ...
  for (const bill of leg.bills) {
    ...
    const card = document.createElement('div'); card.className = 'dm-bill-card';
  ...
  for (const cat of D.BILL_CATEGORIES) {
    ...
    if (!items.length) continue;
    const catEl = document.createElement('div'); catEl.className = 'dm-build-cat'; catEl.textContent = cat.toUpperCase();
  ...
  if (isGov) {
    const g5 = document.createElement('div'); g5.className = 'dm-group';
    ...
    const g6 = document.createElement('div'); g6.className = 'dm-group';
    const activeLaws = leg.activeLawIds.map(id => D.BILLS_BY_ID[id]).filter(Boolean);
  ...
  // legislator roster
  const g7 = document.createElement('div'); g7.className = 'dm-group';
  ...
  for (const l of roster) {
    ...
    const row = document.createElement('div'); row.className = 'dm-legislator-row';
...
function showAppointModal(portfolioId) {
  ...
  box.innerHTML = `<h2>Appoint ${esc(pf.name)}</h2>
    <div class="dm-feed" style="max-height:360px;overflow-y:auto;margin:12px 0">` +
    (eligible.length
      ? eligible.map(l => `<div class="dm-feed-item" data-id="${l.id}" style="cursor:pointer"><b>${esc(l.name)}</b> (${esc(partyName(l.party))}) — competence ${l.competence}, experience ${l.experience}</div>`).join('')
      : '<div class="dm-feed-item">No eligible legislators from the governing party.</div>') +
    `</div><div class="dm-menu-buttons"><button class="dm-btn" data-act="cancel">CANCEL</button></div>`;
...
function renderArchive(body) {
  const tabs = document.createElement('div'); tabs.className = 'dm-archive-tabs';
  ...
  const g = document.createElement('div'); g.className = 'dm-group';
  ...
  if (archiveSubTab === 'elections') {
    g.innerHTML = `<b class="hd">ELECTION HISTORY (${h.elections.length})</b>` +
      (h.elections.slice().reverse().map(e => {
        ...
        return `<div class="dm-archive-row"><b>Year ${year}</b> — ${results}${e.isPlayerGovernment ? ' · <span style="color:var(--green)">You formed Government</span>' : ''}</div>`;
      }).join('') || '<div class="dm-archive-row">No elections recorded yet.</div>');
    ...
    g.innerHTML = `<b class="hd">GOVERNMENTS (${h.governments.length})</b>` +
      (h.governments.slice().reverse().map(gv => {
        ...
        return `<div class="dm-archive-row"><b>${esc(gv.headOfGovernment)}</b> (${esc(partyName(gv.leadParty))}) — Year ${startY}${endY ? ` to Year ${endY}` : ' – present'}</div>`;
      }).join('') || '<div class="dm-archive-row">No governments recorded yet.</div>');
    ...
    for (const bill of list) {
      const def = D.BILLS_BY_ID[bill.billDefId];
      const card = document.createElement('div'); card.className = 'dm-bill-card ' + statusClassOf(bill.status);
    ...
    g.innerHTML = `<b class="hd">YEARLY BUDGET HISTORY (${h.budgets.length})</b>` +
      (h.budgets.slice().reverse().map(b =>
        `<div class="dm-archive-row"><b>Year ${b.year}</b> — Revenue ${fmtMoney(b.revenue)}, Spending ${fmtMoney(b.spending)}, Debt ${fmtMoney(b.debt)}, GDP ${fmtMoney(b.gdp)}, Pop ${fmt(b.population)}</div>`
      ).join('') || '<div class="dm-archive-row">No budget snapshots recorded yet (recorded once per in-game year).</div>');
    ...
    for (const a of D.ACHIEVEMENTS) {
      ...
      const card = document.createElement('div'); card.className = 'dm-bill-card' + (done ? ' passed' : '');
...
function renderTerritory(body) {
  const g1 = document.createElement('div'); g1.className = 'dm-group';
  g1.innerHTML = `<b class="hd">CITIES</b>` + state.cities.map(c => `<div class="dm-feed-item"><b>${esc(c.name)}</b> — capital</div>`).join('');
  body.appendChild(g1);
  ...
  const g2 = document.createElement('div'); g2.className = 'dm-group';
  g2.innerHTML = `<b class="hd">TERRITORY</b><div class="dm-stat-grid">
    <div class="dm-stat-card"><div class="v">${owned}</div><div class="l">TILES OWNED</div></div>
    <div class="dm-stat-card"><div class="v">${Object.values(resources).reduce((a, b) => a + b, 0)}</div><div class="l">RESOURCE DEPOSITS</div></div>
  </div>`;
  body.appendChild(g2);

  const hasBase = state.buildings.some(b => b.built && b.type === 'military_base');
  const g3 = document.createElement('div'); g3.className = 'dm-group';
  const garrisonTotal = Object.values(state.garrison).reduce((a, b) => a + b, 0);
  g3.innerHTML = `<b class="hd">MILITARY</b>
    <div class="dm-stat-grid">
      <div class="dm-stat-card"><div class="v">${state.stats.defense}</div><div class="l">DEFENSE RATING</div></div>
      <div class="dm-stat-card"><div class="v">${garrisonTotal}</div><div class="l">TOTAL UNITS</div></div>
    </div>`;
  if (garrisonTotal) {
    g3.innerHTML += Object.entries(state.garrison).map(([uid, c]) =>
      `<div style="display:flex;justify-content:space-between;font-size:.76rem;padding:4px 0"><span>${D.UNITS_BY_ID[uid].icon} ${D.UNITS_BY_ID[uid].name}</span><b>${c}</b></div>`).join('');
  }
  body.appendChild(g3);

  if (!hasBase) {
    const note = document.createElement('div'); note.className = 'dm-locked-note';
    note.textContent = 'Build a Military Base (Build tab) to train units.';
    body.appendChild(note);
  } else {
    const gq = document.createElement('div'); gq.className = 'dm-group';
    gq.innerHTML = `<b class="hd">PRODUCTION QUEUE (${state.buildQueue.length}/${S.MAX_QUEUE})</b>`;
    gq.innerHTML += state.buildQueue.length
      ? state.buildQueue.map(item => {
          const pct = Math.round((1 - item.ticksLeft / item.totalTicks) * 100);
          return `<div class="dm-queue-item"><span>${D.UNITS_BY_ID[item.unitId].icon}</span><div class="prog"><i style="width:${pct}%"></i></div><span>${item.ticksLeft}t</span></div>`;
        }).join('')
      : '<p style="color:var(--muted);font-size:.76rem">Nothing training.</p>';
    body.appendChild(gq);

    const g4 = document.createElement('div'); g4.className = 'dm-group';
    g4.innerHTML = '<b class="hd">TRAIN UNITS</b>';
    for (const u of D.UNITS) {
      const locked = (u.needsTech && !S.techUnlocked(state, u.needsTech)) || state.buildQueue.length >= S.MAX_QUEUE || state.treasury < u.cost;
      const el = document.createElement('button');
      el.className = 'dm-build-item' + (locked ? ' disabled' : '');
      const lockReason = u.needsTech && !S.techUnlocked(state, u.needsTech) ? ` 🔒 ${D.TECHS_BY_ID[u.needsTech].name}` : '';
      el.innerHTML = `<span class="ic">${u.icon}</span><span class="info"><span class="nm">${u.name}</span><span class="ds">${u.desc}${lockReason}</span></span><span class="cost">${fmtMoney(u.cost)}</span>`;
      if (!locked) el.addEventListener('click', () => { const r = S.queueUnit(state, u.id); if (!r.ok) toast(r.reason, 'warn'); onChange(true); });
      g4.appendChild(el);
    }
    body.appendChild(g4);
  }
  ...
  const g5 = document.createElement('div'); g5.className = 'dm-group';
  g5.innerHTML = '<b class="hd">SELECTED TILE</b>';
  if (!selectedTile) {
    g5.innerHTML += '<p style="color:var(--muted);font-size:.78rem">Click a tile on the map to inspect it.</p>';
  } else {
    const tile = W.tileAt(state.world, selectedTile.x, selectedTile.y);
    if (!tile) { g5.innerHTML += '<p style="color:var(--muted);font-size:.78rem">Off the map.</p>'; }
    else if (tile.owner === 0) { g5.innerHTML += '<p style="color:var(--muted);font-size:.78rem">This is your territory.</p>'; }
    else if (tile.owner === -1) { g5.innerHTML += '<p style="color:var(--muted);font-size:.78rem">Unclaimed territory.</p>'; }
    else {
      const nation = state.nations.find(n => n.id === tile.owner);
      const adjacent = S.isAdjacentToPlayer(state, selectedTile.x, selectedTile.y);
      g5.innerHTML += `<p style="font-size:.78rem;margin-bottom:8px"><b style="color:${nation.color}">${esc(nation.name)}</b> territory</p>`;
      if (nation.eliminated) g5.innerHTML += '<p style="color:var(--muted);font-size:.74rem">This nation has been eliminated.</p>';
      else if (!nation.atWar) g5.innerHTML += '<p style="color:var(--muted);font-size:.74rem">Not at war. Declare war from the World tab to attack.</p>';
      else if (!adjacent) g5.innerHTML += '<p style="color:var(--muted);font-size:.74rem">Not adjacent to your territory — you can only attack bordering tiles.</p>';
      else {
        const panel = document.createElement('div'); panel.className = 'dm-attack-panel';
        const atkPow = Math.round(S.sumGarrisonStat(state.garrison, 'attack') * attackFraction);
        const tiles = Math.max(1, S.tileCountFor(state, nation.id));
        const defPow = Math.round((nation.militaryStrength / tiles) * 6);
        panel.innerHTML = `<div class="vs"><span class="atk">Your strength: ${atkPow}</span><span class="def">Their defense: ~${defPow}</span></div>`;
        const fracRow = document.createElement('div'); fracRow.className = 'dm-frac-row';
        for (const f of [0.25, 0.5, 0.75, 1]) {
          const b = document.createElement('button'); b.className = 'dm-frac-btn' + (attackFraction === f ? ' on' : ''); b.textContent = Math.round(f * 100) + '%';
          b.addEventListener('click', () => { attackFraction = f; renderDash(); });
          fracRow.appendChild(b);
        }
        panel.appendChild(fracRow);
        const launch = document.createElement('button'); launch.className = 'dm-attack-launch'; launch.textContent = '⚔️ LAUNCH ATTACK';
        launch.addEventListener('click', () => {
          const r = S.attackTile(state, selectedTile.x, selectedTile.y, attackFraction);
          if (!r.ok) toast(r.reason, 'warn');
          onChange(true);
        });
        panel.appendChild(launch);
        g5.appendChild(panel);
      }
    }
  }
  body.appendChild(g5);
...
function renderEvents(body) {
  const g = document.createElement('div'); g.className = 'dm-group';
  g.innerHTML = `<b class="hd">EVENT HISTORY</b><div class="dm-feed">` +
    (state.events.log.slice().reverse().map(e => `<div class="dm-feed-item"><b>${esc(e.title)}</b><br>${esc(e.choice)}</div>`).join('') || '<div class="dm-feed-item">No events yet.</div>') +
    `</div>`;
  body.appendChild(g);
}
...
function showEventModal() {
  ...
  const box = $('event-box');
  box.innerHTML = `<div class="icon">${ev.icon}</div><h2>${esc(ev.title)}</h2><p>${esc(ev.desc)}</p><div class="dm-event-choices"></div>`;
  const wrap = box.querySelector('.dm-event-choices');
...
function init(_state, changeCb) {
  ...
  initMap(state);
  document.querySelectorAll('.dm-tab').forEach(t => t.addEventListener('click', () => setTab(t.dataset.tab)));
  refresh();
}
