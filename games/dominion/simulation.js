...
const MAP_W = 48, MAP_H = 32, AI_COUNT = 4, CITY_RADIUS = 10; // large enough that borders reliably touch rivals, so war is always reachable
...
function newGame(opts) {
  ...
  const nations = world.aiStarts.map((s, i) => ({
    ...
    atWar: false, eliminated: false, militaryStrength: 250 + rng() * 350,
  }));
...
function recompute(state) {
  ...
  defense = (defense + law.defenseFlat + sumGarrisonStat(state.garrison, 'defense')) * L.cabinetMultiplier(state, 'defence') * techBonus(state, 'defMult', 'mult');
...
function simulateEconomy(state) {
  ...
  const militaryUpkeep = sumGarrisonStat(state.garrison, 'upkeep') * state.funding.military;
...
function diplomacyAction(state, nationId, action) {
  ...
  if (action === 'trade') { n.tradeAgreement = true; n.relation = clamp(n.relation + 8, 0, 100); notify(state, `Trade agreement signed with ${n.name}.`, 'diplomacy'); }
    ...
    if (n.eliminated) return { ok: false, reason: 'That nation has already been eliminated.' };
    if (n.atWar) return { ok: false, reason: 'Already at war.' };
    n.atWar = true; n.alliance = false; n.nonAggression = false; n.tradeAgreement = false;
    n.relation = clamp(n.relation - 30, 0, 100);
    notify(state, `War declared on ${n.name}!`, 'battle');
  } else if (action === 'peace') {
    if (!n.atWar) return { ok: false, reason: 'You are not at war.' };
    n.atWar = false; n.relation = clamp(n.relation + 5, 0, 100);
    notify(state, `Peace agreed with ${n.name}.`, 'diplomacy');
  }
...
const MAX_QUEUE = 5;
function sumGarrisonStat(garrison, statKey) {
  let s = 0;
  for (const [id, count] of Object.entries(garrison)) { const def = D.UNITS_BY_ID[id]; if (def) s += def[statKey] * count; }
  return s;
}
function queueUnit(state, unitId) {
  ...
  if (def.needsTech && !techUnlocked(state, def.needsTech)) return { ok: false, reason: `Requires ${D.TECHS_BY_ID[def.needsTech].name}.` };
  if (state.buildQueue.length >= MAX_QUEUE) return { ok: false, reason: 'Production queue is full.' };
  if (state.treasury < def.cost) return { ok: false, reason: 'Not enough treasury.' };
  state.treasury -= def.cost;
  const ticks = Math.max(1, Math.round(def.trainTicks * techBonus(state, 'trainTimeMult', 'mult')));
  state.buildQueue.push({ unitId, ticksLeft: ticks, totalTicks: ticks });
  return { ok: true };
}
function advanceMilitaryProduction(state) {
  if (!state.buildQueue.length) return;
  const item = state.buildQueue[0];
  item.ticksLeft--;
  if (item.ticksLeft <= 0) {
    state.garrison[item.unitId] = (state.garrison[item.unitId] || 0) + 1;
    state.buildQueue.shift();
    notify(state, `${D.UNITS_BY_ID[item.unitId].name} training complete.`, 'build');
  }
}

/* ------------------------------------------------------------------ */
/* territorial conquest — attack adjacent enemy-held tiles once at war */
/* ------------------------------------------------------------------ */
function tileCountFor(state, ownerId) {
  let c = 0;
  for (const t of state.world.tiles) if (t.owner === ownerId) c++;
  return c;
}
function isAdjacentToPlayer(state, x, y) {
  for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
    const t = W.tileAt(state.world, x + dx, y + dy);
    if (t && t.owner === 0) return true;
  }
  return false;
}
function checkNationElimination(state, nation) {
  if (nation.eliminated) return;
  if (tileCountFor(state, nation.id) === 0) {
    nation.eliminated = true; nation.atWar = false;
    notify(state, `${nation.name} has been eliminated from the map!`, 'battle');
  }
}
function attackTile(state, x, y, fraction) {
  const tile = W.tileAt(state.world, x, y);
  if (!tile) return { ok: false, reason: 'Invalid tile.' };
  if (tile.owner === 0) return { ok: false, reason: 'You already own this tile.' };
  if (tile.owner === -1) return { ok: false, reason: 'This tile is unclaimed — nothing to conquer.' };
  const nation = state.nations.find(n => n.id === tile.owner);
  if (!nation || nation.eliminated) return { ok: false, reason: 'Invalid target.' };
  if (!nation.atWar) return { ok: false, reason: `You are not at war with ${nation.name}. Declare war first.` };
  if (!isAdjacentToPlayer(state, x, y)) return { ok: false, reason: 'You can only attack territory adjacent to your own.' };
  const totalGarrison = Object.values(state.garrison).reduce((a, b) => a + b, 0);
  if (!totalGarrison) return { ok: false, reason: 'You have no army to send.' };

  const committed = {};
  for (const [uid, count] of Object.entries(state.garrison)) {
    const n = Math.round(count * fraction);
    if (n > 0) committed[uid] = n;
  }
  const atkPower = sumGarrisonStat(committed, 'attack') * (0.85 + Math.random() * 0.3);
  const tiles = Math.max(1, tileCountFor(state, nation.id));
  const defPower = (nation.militaryStrength / tiles) * 6 * (0.85 + Math.random() * 0.3);

  if (atkPower > defPower) {
    tile.owner = 0;
    state.war.tilesCaptured = (state.war.tilesCaptured || 0) + 1;
    nation.militaryStrength = Math.max(0, nation.militaryStrength - defPower * 0.5);
    for (const [uid, count] of Object.entries(committed)) {
      const losses = Math.round(count * 0.08);
      if (losses > 0) { state.garrison[uid] -= losses; if (state.garrison[uid] <= 0) delete state.garrison[uid]; }
    }
    notify(state, `Your forces captured territory from ${nation.name}!`, 'battle');
    checkNationElimination(state, nation);
  } else {
    for (const [uid, count] of Object.entries(committed)) {
      const losses = Math.round(count * 0.35);
      if (losses > 0) { state.garrison[uid] -= losses; if (state.garrison[uid] <= 0) delete state.garrison[uid]; }
    }
    notify(state, `Your assault on ${nation.name}'s territory was repelled.`, 'battle');
  }
  return { ok: true };
}
function aiWarTurn(state, nation) {
  if (!nation.atWar || nation.eliminated) return;
  nation.militaryStrength += nation.gdp * 0.002;
  if (Math.random() > 0.08) return;
  const candidates = [];
  for (let y = 0; y < state.world.h; y++) {
    for (let x = 0; x < state.world.w; x++) {
      const t = state.world.tiles[y * state.world.w + x];
      if (t.owner !== nation.id) continue;
      for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
        const nb = W.tileAt(state.world, x + dx, y + dy);
        if (nb && nb.owner === 0) candidates.push([x + dx, y + dy]);
      }
    }
  }
  if (!candidates.length) return;
  const [tx, ty] = candidates[Math.floor(Math.random() * candidates.length)];
  const defPower = sumGarrisonStat(state.garrison, 'defense') * 0.5 * (0.85 + Math.random() * 0.3);
  const atkPower = nation.militaryStrength * 0.1 * (0.85 + Math.random() * 0.3);
  if (atkPower > defPower) {
    W.tileAt(state.world, tx, ty).owner = nation.id;
    notify(state, `${nation.name} has seized territory from you!`, 'battle');
  }
}
...
return {
  MAP_W, MAP_H, AI_COUNT, ELECTION_INTERVAL, MAX_QUEUE,
  ...
  queueUnit, attackTile, isAdjacentToPlayer, sumGarrisonStat, tileCountFor,
  resolveEvent, diplomacyAction, migrate, computeScore,
};
})();
