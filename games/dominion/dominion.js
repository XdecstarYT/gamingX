/* ============================================================================
   DOMINION — bootstrap: menus, save system, game loop.
   ========================================================================== */
(() => {
'use strict';
const D = window.DomData;
const S = window.DomSim;
const UI = window.DomUI;
const $ = id => document.getElementById(id);

const SAVES_KEY = 'gamingx.dominion.saves';
const HI_KEY = 'gamingx.hi.dominion';
...
function showScreen(id) {
  document.querySelectorAll('.dm-screen').forEach(s => s.classList.remove('active'));
...
function populateSetup() {
  ...
  for (const g of D.GOVERNMENTS) {
    ...
    el.className = 'dm-gov-card' + (g.id === setupGov ? ' on' : '');
    el.innerHTML = `<b>${g.name}</b><span>${g.desc}</span>`;
    el.addEventListener('click', () => { setupGov = g.id; wrap.querySelectorAll('.dm-gov-card').forEach(c => c.classList.remove('on')); el.classList.add('on'); });
    wrap.appendChild(el);
  }
  ...
  for (const p of D.PARTIES) {
    const el = document.createElement('div');
    el.className = 'dm-gov-card' + (p.id === setupParty ? ' on' : '');
    el.innerHTML = `<b style="color:${p.color}">${p.name}</b><span>${p.ideology.replace('-', ' / ')}</span>`;
    el.addEventListener('click', () => { setupParty = p.id; pwrap.querySelectorAll('.dm-gov-card').forEach(c => c.classList.remove('on')); el.classList.add('on'); });
...
function populateSaves() {
  ...
  for (const save of list) {
    const d = save.data;
    const row = document.createElement('div'); row.className = 'dm-save-row';
    ...
    row.innerHTML = `<div class="nm">${escapeHtml(save.name)}<span class="meta">Year ${year} · Pop ${Math.round(d.stats.population)} · Score ${d.score || 0}</span></div>
      <button class="dm-btn" data-act="load">LOAD</button>
      <button class="dm-btn" data-act="del">🗑</button>`;
...
function showPauseModal() {
  ...
  box.innerHTML = `<h2>PAUSED</h2>
    <div class="dm-menu-buttons">
      <button class="dm-btn dm-btn-primary" data-act="resume">▶ RESUME</button>
      <button class="dm-btn" data-act="save">💾 SAVE NATION</button>
      <button class="dm-btn" data-act="exit">🚪 SAVE &amp; EXIT TO MENU</button>
    </div>`;
